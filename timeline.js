document.addEventListener('DOMContentLoaded', function(){
    drawChart("#timelineChart", "https://raw.githubusercontent.com/tonyyao08/HackRice2020/master/eventsdata.json");
    //drawChart("#timelineChart1", "https://raw.githubusercontent.com/c-c-l/D3ITL/master/data/timeline.json");
  });
  
  function drawChart(selector, file_path) {
    const svg = d3.select(selector).append("svg").attr('id','Chart').attr("width", '100%').attr("height", 500);
    d3.json(file_path).then(function(data) {
      ...
      data.map(d => {
        let details = d3.select(selector).append('div').classed('details', true).classed('details-' + d.category.toLowerCase(), true).attr('id', 'details-' + d.id);
